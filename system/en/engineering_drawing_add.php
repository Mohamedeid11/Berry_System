    ...
    if (isset($_POST['submit'])) {
        ...
        if (!empty($errors)) {
            ...
            for($i=0 ;$i<$imageCount ;$i++) {
                ...
                $con->query("INSERT INTO `project_drawing` ( `project_id`, `image`) VALUES ( '$project_id', '$image')");

                $id = mysqli_insert_id($con);
                if (!file_exists("../api/uploads/Engineering_Drawing/" . $id)) {
                    mkdir("../api/uploads/Engineering_Drawing/" . $id, 0777, true);
                }
                $image_path = "../api/uploads/Engineering_Drawing/" . $id . "/" . $image;
                $image_database = "{$sit_url}/api/uploads/Engineering_Drawing/" . $id . "/" . $image;

                if (move_uploaded_file($image_tmp, $image_path)) {
                    $update = $con->query("UPDATE `project_drawing` SET  `image`='$image_database' WHERE `id`='$id'");
                }
