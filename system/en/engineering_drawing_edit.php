    ...
    if (isset($_POST['images_update'])) {
        ...
        if (!empty($errors)) {
            ...
            if (isset($_FILES['image_update']['name']) && !empty($_FILES['image_update']['name'])) {
                ...
                $full_img_path = "../api/uploads/Engineering_Drawing/$imagesID_update" . "/" . $image_name;
                if (file_exists($full_img_path)) {
                    @unlink($full_img_path);
                }

                if (!file_exists("../api/uploads/Engineering_Drawing/" . $imagesID_update)) {
                    mkdir("../api/uploads/Engineering_Drawing/" . $imagesID_update, 0777, true);
                }

                $image_name_update = $_FILES['image_update']['name'];
                $image_tmp_update = $_FILES['image_update']['tmp_name'];

                $image_path = "../api/uploads/Engineering_Drawing/$imagesID_update" . "/" . $image_name_update;
                $image_database = "{$sit_url}/api/uploads/Engineering_Drawing/$imagesID_update" . "/" . $image_name_update;


                if (move_uploaded_file($image_tmp_update, $image_path)) {
                    $update = $con->query("UPDATE `project_drawing` SET `project_id`='$project_id',`image`='$image_database'  WHERE `id`='$imagesID_update'");
                }
                if ($update) {
                ...
                $update = $con->query("UPDATE `project_drawing` SET `project_id`='$project_id' WHERE `id`='$imagesID_update'");
            }
        ...
        <div class="content">
            <div class="container">
                ...
                if ($_GET['imagesID']) {
                    ...
                    $query_select = $con->query("SELECT * FROM `project_drawing` WHERE `id` = '{$get_project_id}' LIMIT 1");
